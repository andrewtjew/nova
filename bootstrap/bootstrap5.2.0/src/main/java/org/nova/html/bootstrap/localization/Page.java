...
public class Page extends BootStrapPage
{
    ...
    public Page() throws Throwable
    {
        ...
        nav.mx(0).d(Display.flex).w(100).justify_content(Justify.end);
        {
            ...
            nav.returnAddInner(new NavLink()).addInner(new Icon(Icons.LOCALE).me(1)).addInner("Locales").href("/StringHandleEditor/viewByLocales").text(StyleColor.light).title("View by Locales");
            nav.returnAddInner(new NavLink()).addInner(new Icon(Icons.HANDLES).me(1)).addInner("Handles").href("/StringHandleEditor/viewByHandles").text(StyleColor.light).title("View by handles");
            nav.returnAddInner(new NavLink()).addInner(new Icon(Icons.ADD).me(1)).addInner("Locale").href("/StringHandleEditor/addLocale").text(StyleColor.light).title("Add locale");
            nav.returnAddInner(new NavLink()).addInner(new Icon(Icons.ADD).me(1)).addInner("Handle").href("/StringHandleEditor/addHandle").text(StyleColor.light).title("Add handle");
            nav.returnAddInner(new NavLink()).addInner(new Icon(Icons.ADD).me(1)).addInner("Enum").href("/StringHandleEditor/addEnum").text(StyleColor.light).title("Add all enum constants");
            nav.returnAddInner(new NavLink()).addInner(new Icon(Icons.UNDEFINED).me(1)).addInner(" Undefines").href("/StringHandleEditor/nextUndefined").text(StyleColor.warning).title("Check for undefined handles");

            nav.returnAddInner(new NavLink()).addInner(new Icon(Icons.DOCUMENTATION)).href("https://docs.oracle.com/javase/7/docs/api/java/util/Formatter.html").target(target._blank).text(StyleColor.light).title("Link to Format string documentation");
            nav.returnAddInner(new NavLink()).addInner(new Icon(Icons.SETTINGS)).href("/StringHandleEditor/settings").text(StyleColor.light).title("Locale and enum settings").ms_auto();
        }
